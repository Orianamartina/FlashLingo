...
function checkCard(card, input, time){
    let points = 0
    ...
    cards.forEach(card => {
        ...
        if (card.color === "green"){
            if(card.points === -1 || card.points === 0){
                delete card.color
                delete card.points
                yellow_cards.push(card)
            }else{
                delete card.color
                delete card.points
                green_cards.push(card)
            }
        }
